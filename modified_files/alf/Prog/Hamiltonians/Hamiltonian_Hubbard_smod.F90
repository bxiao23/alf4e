...
!               &                   N_part, N_FL,  WF_L, WF_R)
          Call Predefined_TrialWaveFunction_Hubbard(Lattice_type ,Ndim,  List,Invlist,Latt, Latt_unit, &
               &                   Ham_Chem, N_part, N_FL,  WF_L, WF_R)
...
!--------------------------------------------------------------------
        Subroutine Ham_V
          ...
          Integer :: nf, nfI, I, I1, I2,  nc,  J, no,  N_ops
          ...
          If ( Mz )  Then
             ...
             enddo
          ! --------------------------------------------------
          ! below: the relevant part (non-Mz Hamiltonian)
          else
             Allocate(Op_V(N_ops,N_FL))
             nc = 0
             Do I1 = 1,Latt%N
                do no = 1, Latt_unit%Norb
                  ...
                  do nfI = 1,N_FL
                     ...
                     if (abs(Ham_U_vec(no)) > Zero ) then
                        if (Continuous) then
                           Call Predefined_Int_U_SUN_continuous_HS(  OP_V(nc,nfI), I, N_SUN, DTAU, Ham_U_vec(no)  )
                        else
                           Call Predefined_Int_U_SUN              (  OP_V(nc,nfI), I, N_SUN, DTAU, Ham_U_vec(no)  )
                        endif
                     endif
                  enddo
                Enddo
